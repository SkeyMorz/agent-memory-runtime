...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/layers-5%20%2B%20REST%20API-orange" alt="Layers">
  <img src="https://img.shields.io/badge/tests-37%20passed-brightgreen" alt="Tests">
</p>

- 📝 **Project Status:** v0.6 — full 5-layer pipeline + REST API done, production hardening next
- 🏗️ **Architecture:** ingestion → retrieval → ranking → consolidation → context → REST API
...
| REST API | 7 个端点，FastAPI + Swagger 文档 | `memory/api.py` | ✅ done |

---

## 📰 News

- **2026-05-28** — v0.6 released: FastAPI REST service — 7 endpoints wrapping all 5 layers, 37 tests passing.
- **2026-05-28** — v0.5 released: ContextBuilder with 3 templates (default / compact / system_prompt), token budget control.
...
├── pyproject.toml
├── memory/
│   ├── api.py                      # FastAPI app — 7 REST endpoints
│   ├── ingestion/
...
# 打开 Swagger 文档 → http://127.0.0.1:8000/docs
```
...
# 去重
curl -X POST http://127.0.0.1:8000/consolidate \
  -H "Content-Type: application/json" \
  -d '{"threshold": 0.45}'
...
curl -X POST http://127.0.0.1:8000/context \
  -H "Content-Type: application/json" \
  -d '{"query": "ramen", "template": "default"}'
```
...
| `GET` | `/search?q=&strategy=&top_k=` | Semantic search + rank |
...
| v0.6 | FastAPI service — 7 REST endpoints, Swagger docs | ✅ done |
| v1.0 | Production-ready — persistent storage, embedding retrieval | 🔜 next |
