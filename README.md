...
Agent 查询 / API 请求
        ...
        ▼
┌──────────────────┐
│   Retrieval      │  TF-IDF / Embedding 双引擎语义搜索
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│    Ranking       │  语义相关性 + 时间新鲜度混合排序
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  Consolidation   │  检测相似记忆 → 合并去重（可选）
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│ Context Builder  │  结构化结果 → 自然语言段落 → 注入 prompt
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│     Agent        │  prompt + 记忆上下文 → 个性化回复
└──────────────────┘
```
