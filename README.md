...
| 层 | 做什么 | 代码位置 | 状态 |
|----|--------|----------|------|
| Ingestion | 结构化存储 + JSON 持久化（auto-save） | `memory/ingestion/` | ✅ done |
| Retrieval | 双引擎：TF-IDF + Embedding（sentence-transformers） | `memory/retrieval/` | ✅ done |
| Ranking | 相关性 + 新鲜度混合排序（3 种策略） | `memory/ranking/` | ✅ done |
| Consolidation | 合并相似记忆，去重去噪 | `memory/consolidation/` | ✅ done |
| Context Builder | 检索结果 → prompt 文本（3 种模板） | `memory/context/` | ✅ done |
| REST API | 9 个端点，FastAPI + Swagger 文档 | `memory/api.py` | ✅ done |
