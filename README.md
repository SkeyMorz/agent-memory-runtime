...
昨天告诉 AI"我在减肥"，今天它不会推荐炸鸡。用户偏好、重要日期、说过的事情——全部持久化到本地文件，重启不丢、换设备可迁移。

### 只筛选相关记忆塞入 prompt

传统做法是把历史对话全塞进 prompt，结果就是 Token 费用爆炸、AI 注意力涣散。我们的做法是：每当 AI 需要回答一个问题时，先语义搜索最相关的几条记忆，只把这几条拼进去。记得全，但花得少。

### 自检相似记忆，合并去重

人会在不同时间说相似的话——"我喜欢日本料理"和"我爱吃日料"。系统自动检测相似记忆，合并去重。记忆库不会无限膨胀，也不会同一条信息存了好几份。

### 无需更换框架

不管你用 LangChain、AutoGPT、还是自己写的 Agent 循环——import 一个 class，调两个方法（`add` + `search`），就接上了。如果连 Python 都不想写，起一个 HTTP 服务，任何语言都能调。

### 检索引擎可替换

当前用 TF-IDF 做语义检索，不需要向量数据库和 Embedding API，零额外成本就能跑。当你需要更强的语义理解时，一行参数切成 Embedding 引擎——接口不变，能力升级。
