...
当前用 TF-IDF 起步——零外部依赖、零 API 密钥、零成本，Recall@5 50.9%。需要更强的语义理解时，一个参数切换到 Embedding 引擎，Recall@5 82.2%。接口不变，能力升级，不需要重构。
...
- **2026-05-30** — 检索升级：字符 n-gram TF-IDF（Recall@5 50.9%）+ Jaccard 安全检查（合并准确率 4/5）。Embedding 引擎：Recall@5 82.2%。
- **2026-05-29** — v1.0 发布：JSON 持久化 + EmbeddingRetriever（sentence-transformers），46 个测试全部通过。
...
数据集：**25 条记忆**，8 个类别，**20 组标注查询**。运行 `python -m benchmarks.evaluate` 可复现。

### 检索性能

| 引擎 | Recall@5 | Recall@10 | MRR | 延迟 |
|------|----------|-----------|-----|------|
| TF-IDF（字符 n-gram） | 50.9% | 67.2% | 0.526 | 3.2 ms |
| Embedding（all-MiniLM-L6-v2） | 82.2% | 93.3% | 0.925 | 1503 ms |

TF-IDF 使用字符级 n-gram 捕获部分词匹配（如 "ramen" 和 "ramens" 共享子串）。当查询词与记忆词有重叠时检索效果好；当查询和记忆用完全不同的词表达同一概念时，只有 Embedding 引擎能弥合差距。
...
| 场景 | 结果 |
|------|------|
| 近似重复（"Japanese food ramen sushi" vs "Japanese cuisine ramen sushi"） | 已合并 |
| 不同主题（"Python developer" vs "likes Japanese food"） | 已保留 |
| 三条相似、两条合并（保守策略） | 3→2（Jaccard 安全检查阻止误合并） |
| 四条不同事实 | 全部保留 |
| 相似徒步句子 | 正确合并 |

Jaccard 相似度安全检查阻止了句式相同但事实不同的句子被误合并。代价是少部分真实重复未被合并，换取零误合并。
...
46 个测试，覆盖 6 个模块：API（14）、持久化（6）、检索（4）、排序（6）、合并（8）、上下文（8）。1 个 Embedding 测试需要 `sentence-transformers`，默认跳过。
